 ...
 * adfSetCurrentTime
 *
 * sets the current system date and time for ADF
 */
void adfSetCurrentTime(int year, int mon, int day, int hour, int min, int sec)
{
    setTime(hour, min, sec, day+1, mon, year);
}
...
adfGiveCurrentTime( void )
{
    ...
    r.year=local.Year+70;         /* since 1900 */
    r.mon=local.Month;
    r.day=local.Day;
    r.hour=local.Hour;
    r.min=local.Minute;
...
void dumpBlock(uint8_t *buf)
{
	...
	for(i=0; i<32; i++) {
		Serial.printf("%5x ",i*16);
        for (j=0; j<4; j++) {
			Serial.printf("%08x ",Long(buf+j*4+i*16));
		}
        Serial.printf("    ");
        for (j=0; j<16; j++)
