...
void adfDeviceInfo(struct Device *dev)
{
	...
	Serial.printf("[ADFlib]Cylinders   = %ld\n",dev->cylinders);
    Serial.printf("[ADFlib]Heads       = %ld\n",dev->heads);
    Serial.printf("[ADFlib]Sectors/Cyl = %ld\n\n",dev->sectors);
	if (!dev->isNativeDev)
        Serial.printf("[ADFlib]Dump device\n\n");
    else
        Serial.printf("[ADFlib]Real device\n\n");
    Serial.printf("[ADFlib]Volumes     = %d\n\n",dev->nVol);
/*
    switch(dev->devType){
    case DEVTYPE_FLOPDD:
...
*/
    ...
    for(i=0; i<dev->nVol; i++) {
        if (dev->volList[i]->volName)
            Serial.printf("[ADFlib]%2d :  %7ld ->%7ld, \"%s\"", i,
			dev->volList[i]->firstBlock,
			dev->volList[i]->lastBlock,
			dev->volList[i]->volName);
        else
            Serial.printf("[ADFlib]%2d :  %7ld ->%7ld", i,
			dev->volList[i]->firstBlock,
			dev->volList[i]->lastBlock);
