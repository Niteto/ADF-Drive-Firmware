...
//Serial.printf("hashTable=%d nexthash=%d\n",parent.hashTable[hashVal], entry.nextSameHash);
    ...
    if (entry.secType==ST_FILE) {
		...
		adfSetBlockFree(vol, nSect); //added by Nick
        if (adfEnv.useNotify)
...
void adfFreeDirList(struct List* list)
{
    ...
    root = cell = list;
    while(cell!=NULL) {
        adfFreeEntry((Entry*)cell->content);
        if (cell->subdir!=NULL)
    ...
    struct bEntryBlock *entry, SECTNUM *nUpdSect)
{
    ...
    nSect = ht[hashVal];
/*Serial.printf("name=%s ht[%d]=%d upper=%s len=%d\n",name,hashVal,nSect,upperName,nameLen);
Serial.printf("hashVal=%d\n",adfGetHashValue(upperName, intl ));
if (!strcmp("espa�a.country",name)) {
int i;
for(i=0; i<HT_SIZE; i++) Serial.printf("ht[%d]=%d    ",i,ht[i]);
}*/
    if (nSect==0)
        return -1;
...
{
    Serial.printf("[ADFlib]%-30s %2d %6ld ", entry->name, entry->type, entry->sector);
    Serial.printf("%2d/%02d/%04d %2d:%02d:%02d",entry->days, entry->month, entry->year,
        entry->hour, entry->mins, entry->secs);
    if (entry->type==ST_FILE)
        Serial.printf("%8ld ",entry->size);
    else
        Serial.printf("         ");
    if (entry->type==ST_FILE || entry->type==ST_DIR)
        Serial.printf("%-s ",adfAccess2String(entry->access));
    if (entry->comment!=NULL)
...
#ifdef debugadf		
        Serial.printf("[ADFlib]nameLen=%d, commLen=%d, name=%s sector%ld\n",
