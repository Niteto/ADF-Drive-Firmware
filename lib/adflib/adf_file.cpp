 ...
 *
 */
void adfFlushFile(struct AFile *file)
{
 ...
 *
 */
void adfFileSeek(struct AFile *file, uint32_t pos)
{
 ...
 *
 */ 
struct AFile* adfOpenFile(struct Volume *vol, char* name, char *mode)
{
    struct AFile *file;
    SECTNUM nSect;
 ...
 *
 */
void adfCloseFile(struct AFile *file)
{
 ...
 *
 */
int32_t adfReadFile(struct AFile* file, int32_t n, uint8_t *buffer)
{
    int32_t bytesRead;
...
/*puts("adfReadFile");*/
    ...
    if (isOFS(file->volume->dosType))
        ...
        dataPtr = (uint8_t*)file->currentData;
 ...
 *
 */
BOOL adfEndOfFile(struct AFile* file)
{
    return(file->eof);
}
 ...
 *
 */
int32_t adfWriteFile(struct AFile *file, int32_t n, uint8_t *buffer)
{
...
/*puts("adfCreateNextFileBlock");*/
    blockSize = file->volume->datablockSize;
    data = (bOFSDataBlock*)file->currentData;
...
#ifdef LITT_ENDIAN
        swapEndian((uint8_t*)data, SWBL_DATA);
#endif
...
/*        *(int32_t*)(buf+20) = swapLong((uint8_t*)&newSum);*/
        ...
        adfWriteBlock(vol,nSect,(uint8_t*)data);
    }
